...
fn decode(values: Vec<&str>) -> Vec<HashSet<char>> {
    ...
    for i in values.iter() {
        if i.len() == 5 {
            if ((i.chars().collect::<HashSet<char>>())
                .difference(&seven)
                .collect::<HashSet<&char>>())
            .len()
                == 2
            ...
            } else if (i.chars().collect::<HashSet<char>>())
                .difference(&four)
                .collect::<HashSet<&char>>()
                .len()
                == 2
...
fn main() -> Result<(), Error> {
    ...
    for line in reader.lines() {
        let lines = line?;
        let split: Vec<&str> = lines.split(" | ").collect();
        let decoded_numbers: Vec<HashSet<char>> = decode(split[0].split_whitespace().collect());
        ...
        for i in split[1].split_whitespace() {
            ...
            for (index, j) in decoded_numbers.iter().enumerate() {
                if num_char.difference(j).collect::<HashSet<_>>().is_empty()
                    && num_char.len() == j.len()
                {
